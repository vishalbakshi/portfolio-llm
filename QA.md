...
Video: https://youtu.be/JXzEMj_8SyI
...
Research like the TinyStories and "Small Language Models are Equation Reasoners" papers showed that small models can exhibit reasoning if the data format is right. His own experiments confirmed this: he fine-tuned TinyStories models as small as 8M parameters on the financial_phrasebank sentiment dataset and achieved 86% test set accuracy, which was 24% higher than the most frequent class (`negative` sentiment at 62%). See his blog post (https://vishalbakshi.github.io/blog/posts/2024-08-22-tinystories-1m-finetune/).
...
**Answer:** It's custom because standard benchmarks like GLUE often require specific output formats, while he wanted to measure more complex, free-form capabilities like grammar, creativity, plot, reasoning, and context-tracking. His framework is designed to score these dimensions, inspired by the evaluation paradigm in the TinyStories paper. 
...
Finally, dLLMs introduce a fascinating new paradigm. Instead of slow, autoregressive generation, they iteratively improve an entire sequence of text at once, resulting in very fast inference.
